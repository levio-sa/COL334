...
// Function to write congestion window size in file
static void
...
// Function to calculate dropped packets
// A variable pdrop is incremented by 1 whenever a packet gets dropped
static void
...
int main(int argc, char *argv[]){
    ...
    CommandLine cmd;
    cmd.AddValue("TcpVersion", "Version of TCP", TcpVersion); // Add a command line option
    cmd.Parse (argc, argv);
    ...
    TypeId tid = TypeId::LookupByName ("ns3::"+TcpVersion);
    ...
    Ptr<OutputStreamWrapper> Cwndstream = asciiTraceHelper.CreateFileStream (TcpVersion+".cwnd");
    ns3TcpSocket->TraceConnectWithoutContext ("CongestionWindow", MakeBoundCallback (&CwndChange, Cwndstream)); // For congestion window

    Ptr<OutputStreamWrapper> Pdropstream = asciiTraceHelper.CreateFileStream (TcpVersion+".pdrop");
    devices.Get (1)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop, Pdropstream)); // For packet drop

    Simulator::Stop (Seconds (30));
    Simulator::Run ();
    Simulator::Destroy ();

    NS_LOG_UNCOND ("Packets dropped " << pdrop);
